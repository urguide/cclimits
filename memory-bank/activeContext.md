...
- **2026-08-10**: **Grok (xAI) coding-credit usage added.** Official `xai-org/grok-build` source revealed the internal `/v1/billing?format=credits` endpoint and its required session/user/version headers; live account probe returned total + per-product percentages and a weekly reset. Grok now appears in oneline and detailed/JSON output. `~/.grok/auth.json` remains read-only from cclimits; when the tmux wrapper sees an expired session it delegates safe refresh to official `grok models` and retries. Compact/tmux uses ceiling values, tight underscore separators and collapsed `(7d)` / `(16h)` / `(35m)` resets. A stable last-good display cache prevents status flicker during cold/background refresh. 255 → 290 tests

...
## 2026-08-17: `CCLIMITS_TMUX_SEP` — Stylable Provider Separator

- **Motivation**: `--compact` joins providers with `_` to save columns, which reads as a faint underscore among the digits. tmux status text accepts inline `#[...]` style sequences, so the separator can instead be the same coloured bar (`┃`, `#ff8800`) already used by `pane-active-border-style` and `window-status-format`, making the segment boundaries scan at a glance.
- **Implemented in the wrapper, not in `--compact`**: the separator is a tmux-specific display concern and its value is tmux markup. A `--separator` flag on `cclimits` would put tmux style sequences inside a general-purpose CLI *and* inside the cache; a wrapper-level substitution leaves `lib/cclimits.py` untouched.
- **Substituted on serve, never on write** — the load-bearing decision. The wrapper's last-good protection greps refreshed output for `❌|⏰|ERR|:expired(_|$)`. Had the separator been written into the cache, an arbitrary user string would defeat that `_` anchor and an outage would start overwriting good readings — the same frozen-value class of bug as the 2026-08-10 `no key` entry. Serving instead of writing also means a separator change lands on the next redraw rather than after the refresh TTL. Covered by `test_separator_is_not_written_to_the_cache` and `test_separator_does_not_break_transient_failure_protection`.
- **Literal insertion, found by test not by review**: the obvious `${line//_/$CCLIMITS_TMUX_SEP}` is wrong on bash 5.2+, which expands an unquoted `&` in a substitution replacement to the matched text — so `CCLIMITS_TMUX_SEP='#[fg=red]&'` silently emitted a stray `_`. `sed` is worse: it reinterprets `&`, `\`, and the `|` many users would pick as their bar glyph. Final form consumes one `_`-delimited field at a time (`${rest%%_*}` / `${rest#*_}`) and passes the separator as a printf argument, so it stays byte-for-byte literal. `test_separator_is_inserted_literally` exercises `#`, `|`, `&` and `\` together and fails against the parameter-expansion version.
- **Style must be restored, not just applied**: a tmux `#[...]` style persists to end of line, so `#[fg=#ff8800]┃` alone recolours every provider after the first separator. The documented value ends with `#[fg=cyan]` to return to the status-right colour.
- **Verification**: rendered in a live tmux 3.7b (nested server, `capture-pane -e`) — the bar draws in truecolor `38;2;255;136;0` while surrounding text stays `36` (cyan). `status-right-length` truncates on rendered width, not on style bytes. Also confirmed the feature composes with the same-day `CCLIMITS_TMUX_ERROR` work: a live `--watch` loop emits separator-styled lines normally and still switches to `cclimits!` when its own file is replaced mid-run. A tmux-native `#{s|_|...|:}` approach was tried first and rejected — it cannot wrap a `#()` job directly, and the `#{E:}` indirection needed to make it work leaves the separator invisible to the wrapper's own failure detection.
- **Tests**: 306 → 311. Three of the five new tests fail against the pre-change wrapper (verified by stripping `serve()` and reverting the call sites); the other two are invariants (cache stays raw, opt-in output byte-identical) that must hold both before and after.
- **Files**: `bin/cclimits-tmux`, `tests/test_tmux_wrapper.py`, `README.md`, `memory-bank/deltas.md`, `memory-bank/progress.md`

## 2026-08-17: Status Line Froze on `cclimits...` — Corrupt Install Was Indistinguishable From a Cold Cache

- **Symptom**: the tmux status line sat on the `cclimits...` placeholder indefinitely.
- **Root cause (environment)**: `/usr/local/bin/cclimits-tmux` was a 461-byte Varnish **503 error page**, not a shell script (`file` reported `HTML document`). The user had installed via the README's `curl -o` recipe; `curl -o` writes the body of an HTTP error response to the output file **and still exits 0**, so a CDN hiccup silently produced an executable HTML file. Confirmed the mechanism directly: `curl -s -o f URL/does-not-exist` → exit 0, file contains `404: Not Found`; adding `-f` → exit 22, no file created. `/usr/local/bin/cclimits` (the Python script) was byte-identical to the repo and unaffected — only the wrapper was clobbered.
- **Why it presented as a freeze rather than an error**: the `--watch` loop had been running since before the overwrite, so its body was already parsed into memory and kept ticking. Only the `"$0"` child invocations failed. The loop tested `[ -n "$watch_out" ]` alone, so a failed invocation was indistinguishable from an empty one and it printed the reassuring cold-cache placeholder forever. Cache forensics confirmed the data was fine: `*.cache` held a valid `:0%/58%(3h/2d)_…` line stamped one minute *before* the wrapper's mtime.
- **Fix 1 — README**: `curl -o` → `curl -fLo` for both files, with an explicit warning about the exit-0-on-error behaviour. Also documented that `-f` cannot catch a *truncated* download (that is a successful 200) and added a post-install `ast.parse` + `bash -n` verification snippet.
- **Fix 2 — wrapper**: the watch loop now branches on the exit status of `"$0"`, emitting `CCLIMITS_TMUX_ERROR` (default `cclimits!`) for a failed invocation and reserving `CCLIMITS_TMUX_PLACEHOLDER` for genuinely empty output. Uses `${VAR-default}` for consistency with the placeholder, and redirects the child's stderr to avoid spamming the tmux server log every tick.
- **Tests**: 294 → 296 (306 total collected with `requests` installed). The two new tests corrupt a *copy* of the wrapper **after** its loop is live, reproducing the in-memory-loop timing — an earlier draft that corrupted the file before launch was wrong and failed, since bash then never read the loop at all and exited immediately. Confirmed non-vacuous by reverting the loop to the old logic: both fail, both pass again after restoring.
- **Pre-existing failures ruled out**: 8 `test_http.py` failures in the scratch venv were `ModuleNotFoundError: requests`, reproduced identically under `git stash`, and cleared by installing `requests`.
- **Files**: `bin/cclimits-tmux`, `tests/test_tmux_wrapper.py`, `README.md`, `memory-bank/deltas.md`, `memory-bank/progress.md`
...
- **Opt-in, not a default**: a Nerd Font glyph without a patched font renders as a replacement box, which is strictly worse than the word it replaced. cclimits is a public npm package, so the default output is unchanged and `test_default_output_is_unchanged` guards that.
- **Glyph selection was verified against the installed font, not guessed**: parsed `cmap` (format 4/12) and `post` v2.0 tables of `VictorMonoNerdFontMono-Regular.ttf` (Nerd Font 3.5.0) directly to confirm each codepoint is actually present and to recover its Nerd Font name. Claude and Codex have real brand glyphs (`cod-claude` U+EC82, `cod-openai` U+EC81). **There is no xAI/Grok glyph in Nerd Font 3.5.0** — searched `xai|grok|twitter|alpha_x`; Grok uses `cod-twitter` (U+EB72) as the closest brand proxy. The remaining six use Material Design glyphs; Gemini's `md-star_four_points` happens to match Google's actual Gemini mark.
- **Implemented in `print_oneline()`, not in the renderers**: all nine renderers emit `"{label}: ..."` via the shared `_fmt_*` helpers, so a single prefix swap covers every provider. Threading an icon through `_make_str_pct_renderer` / `_make_balance_renderer` closures would have added a parameter to five call layers for a purely presentational change. `_apply_icon_label()` is anchored with `startswith`, not `replace`, so a label recurring inside a value (`Grok: 5% Grok-4`) is untouched.
- **Ordering matters**: the swap runs *before* `_compact_oneline()` so that function's existing `": " → ":"` tightening applies to the icon prefix too. The failure branch builds its label independently of the renderers, so it needed a separate change — and its own test, or it would have kept printing words next to glyph-labeled successes.
- **Latent trap in the wrapper, caught before shipping**: `bin/cclimits-tmux` triggers Grok's safe auto-refresh on `grep -qE 'Grok:(expired|⏰)'`. Renaming the label to a glyph would have made that pattern permanently unmatchable, silently disabling the recovery path — the status line would sit on `expired` until the next manual `grok` run, with nothing indicating the mechanism had stopped firing. The pattern now accepts both spellings via `GROK_ICON=$'\ueb72'`. Confirmed non-vacuous: the new test fails against the old single-label pattern.
- **Tests**: 294 → 303. `TestOnelineIcons` covers the compact line and its exact column saving, the opt-in regression guard, non-compact spacing, glyph-labeled failure states, single-codepoint + uniqueness invariants over the whole registry, and the two `_apply_icon_label` anchoring edge cases; `test_expired_grok_is_refreshed_when_labeled_by_icon` covers the wrapper trap.
- **Files**: `lib/cclimits.py`, `bin/cclimits-tmux`, `tests/test_output.py`, `tests/test_tmux_wrapper.py`, `README.md`, `memory-bank/deltas.md`, `memory-bank/progress.md`

## 2026-08-10: tmux Wrapper — `no key` Froze the Status Line, Cold Cache Showed tmux's Own Error

- **Symptom**: the status line rendered tmux's internal marker `<'~/.local/bin/cclimits-tmux --watch' not ready>`. Source confirmed in tmux `format.c` `format_job_get()`: a job that is still alive but has produced no line for over a second gets that placeholder. So `--watch` was running yet emitting nothing.
- **Root cause of the empty output (environment, not code)**: the tmux server's PATH did not include `~/.local/bin`, so the wrapper's third resolution step (`PATH` lookup of `cclimits`) failed with exit 127. The second step didn't help either — `~/.local/bin/cclimits-tmux` was a *copy*, not a symlink, so `readlink -f` resolved to itself and the sibling `../lib/cclimits.py` probe looked for a nonexistent `~/.local/lib/cclimits.py`. Confirmed by cache forensics (only `.lease`/`.lock` present, never a `.cache` body) and by replaying `--watch` under `env -i` with the tmux server's exact PATH (empty output, same file pattern). Fixed by the user out-of-band; the resolution chain itself was deliberately left unchanged.
- **Bug 1 — `no key` froze every refresh**: the last-good protection at `bin/cclimits-tmux` treated `🔑`/`no key` as a failure alongside `❌`/`⏰`/`ERR`. But a missing credential is a *stable* configuration state, not a transient outage — retrying can never change it. With `--grok` enabled and no Grok login, `Claude:…_Codex:…_Grok:no key` matched the failure regex, so every single refresh was rejected and the Claude/Codex percentages would have stayed pinned to the first reading forever. Verified live: the data cache mtime did not advance across a forced lease cycle. The failure regex is now `❌|⏰|ERR|:expired(_|$)` — transient states only. This is the same frozen-percentage class of bug as the 2026-08-06 entry, but triggered by `no key` rather than `expired`.
- **Bug 2 — cold cache emitted nothing**: `--watch` called `"$0"` and passed its output straight through, so before the first successful lookup it printed empty lines and tmux substituted `<'...' not ready>`. The loop now captures the output and falls back to `CCLIMITS_TMUX_PLACEHOLDER` (default `cclimits...`), so tmux always receives a complete line. The default uses `${VAR-default}` not `${VAR:-default}` so an intentionally empty placeholder (accepting tmux's marker) is honoured.
- **Verified the capture doesn't reintroduce blocking**: `out=$(cclimits-tmux)` returns in ~6ms even when the underlying cclimits sleeps 3s, because the background refresh subshell already redirects stdout to `/dev/null` and so does not hold the command substitution's pipe open.
- **Tests**: 291 → 294. New `test_missing_credentials_do_not_block_refresh`, `test_watch_mode_emits_placeholder_on_cold_cache`, `test_watch_mode_placeholder_can_be_disabled`; `test_watch_mode_keeps_emitting_complete_lines` now also asserts no blank emission and that the placeholder never interleaves with real data. All three new tests were confirmed to fail against the pre-fix wrapper (`git stash`) — not vacuous.
- **Files**: `bin/cclimits-tmux`, `tests/test_tmux_wrapper.py`, `README.md`, `memory-bank/deltas.md`, `memory-bank/progress.md`

## 2026-08-10: Grok (xAI) Coding-Credit Usage

- **Correction after official source review**: the first pass incorrectly concluded that Grok had no quota API. Searching guessed public-style paths (`/usage`, `/quota`, `/limits`) and URL-shaped binary strings missed a path assembled at runtime. The public `xai-org/grok-build` source has an `x.ai/billing` extension in `crates/codegen/xai-grok-shell/src/extensions/billing.rs`, which constructs `GET {cli_proxy}/billing?format=credits` and sends the Grok session token plus `X-XAI-Token-Auth: xai-grok-cli`, `x-userid`, `x-grok-client-version`, and `x-grok-client-mode`. Local `~/.grok/logs/unified.jsonl` also contained real `billing: fetched credits config` snapshots. Live probe returned HTTP 200 with total 55%, GrokBuild 53%, GrokChat 2%, weekly period and prepaid/on-demand fields.
- **Implementation**: `get_grok_usage()` now calls `https://cli-chat-proxy.grok.com/v1/billing?format=credits`, parses `creditUsagePercent`, weekly/monthly `currentPeriod`, `productUsage`, prepaid balance, on-demand cap/used, unified-billing flag and top-up method. Legacy `monthlyLimit` + `used` fields remain a fallback. `/user?include=subscription` is now only a metadata fallback for plan/account/CLI-access when the raw billing response omits the CLI's remote-settings enrichment.
- **Oneline**: Grok has a real custom renderer: `Grok: 55% (7d) ✅ ↻6d4h`. The period comes from the server and can be `7d` or `monthly`.
- **Credentials**: `~/.grok/auth.json` is keyed by `"<oidc_issuer>::<client_id>"`, not flat; each entry holds `key` (access-token JWT), `refresh_token`, `expires_at`. `_read_grok_auth_file()` picks the entry with the latest expiry when several scopes exist. Env fallback is `GROK_ACCESS_TOKEN` only — `XAI_API_KEY` is a different pay-per-token product and cannot authenticate this billing endpoint.
...
- **Tests**: 291 total — Grok coverage includes scoped-key parsing, installed-version discovery/env override, exact required headers, weekly/monthly periods, current + legacy response shapes, product split, settings/user metadata enrichment, auth failures, detailed/oneline rendering and compact reset collapsing; wrapper E2E tests cover cold-view continuity, official Grok delegation, last-good preservation and persistent watch output.
...
- **tmux flicker + Grok refresh follow-up**: the wrapper now maintains a stable display cache independent of argument-keyed data caches, so the segment never goes blank while the first background lookup for a new view runs. Compact `expired` / `no key` outputs now receive the same last-good protection as emoji failures. On `Grok:expired`, the wrapper invokes official `grok models` once in the background and reruns cclimits; this exercises xAI's own `AuthManager`, rotating-refresh-token lock and atomic write-back without consuming inference credits. Added shell-level end-to-end tests with fake cclimits/Grok executables.
- **Actual periodic flicker root cause**: instrumentation showed the wrapper was stable (100/100 calls, 12–35ms, always 48–49 bytes), while the direct API refresh took ~0.93s only in the background. tmux 3.7b format tracing showed a short-lived `#()` job temporarily resolves to an empty string whenever tmux restarts it. Added `cclimits-tmux --watch`: one persistent process emits a complete cached line every 2s, so tmux retains the prior line between emissions. Verified one PID stayed alive across 12 status intervals and standalone watch output remained nonempty.
...
- **Problem**: nothing refreshes `~/.claude/.credentials.json` while Claude Code isn't running. Claude's access token lasts ~8h, so an overnight gap made every lookup 401 → `Token expired`. In the tmux status line that surfaces as a *frozen percentage*, not an error: `bin/cclimits-tmux` deliberately refuses to overwrite a good cached reading with a failing one, so it kept serving a 17-hour-old value with no staleness hint (observed: statusline stuck at `Claude: 10%/11%` while real usage was 65%).
- **Fix**: `ensure_claude_token()` / `ensure_codex_credentials()` redeem the stored refresh token when the access token has expired, and write the result back to the vendor's own credential file so the CLI picks it up too. Both are also called with `force=True` for a single retry when the usage API answers 401 (revoked token, clock skew, stale copy).
- **Claude specifics**: endpoint `platform.claude.com/v1/oauth/token`, public client id `9d1c250a-…` (both lifted from the Claude Code bundle), JSON body + `anthropic-beta: oauth-2025-04-20`. Expiry comes from `claudeAiOauth.expiresAt`. Anthropic **does** rotate the refresh token on every exchange — confirmed by a live forced refresh, which returned a different `refreshToken` while leaving `refreshTokenExpiresAt` untouched (so the unchanged expiry is *not* evidence of a stable token, as first assumed). Write-back is therefore mandatory here too.
- **Codex specifics**: endpoint `auth.openai.com/oauth/token`, client id taken from the user's own access-token JWT (`client_id` claim) with `app_EMoamEEZ73f0CkXaXp7hrann` as fallback, scope `openid profile email`. `auth.json` records no expiry, so it's read from the token's `exp` claim (~10d life). OpenAI **does** rotate the refresh token, so write-back is mandatory — `refresh_token`, `id_token`, the `account_id` re-derived from the new id_token, and an updated `last_refresh` all go back to disk.
- **Safety**: new `write_json_secure()` (atomic, temp created 0600 via `os.open`, never world-readable) now backs the Gemini write too; `credential_lock()` serializes refresh+write across cclimits processes via a `.lock` sibling with a 3s non-blocking timeout (a status line must not block), re-reading the file under the lock. Every non-token field is preserved. macOS Keychain creds stay read-only (no safe write path) — documented. Opt out with `CCLIMITS_NO_TOKEN_REFRESH=1`.
- **Interop lock (Claude)**: because rotation is real, a polite race isn't enough — `claude_refresh_lock()` acquires Claude Code's *own* guard, the proper-lockfile directory `<config-dir>/.oauth_refresh.lock` (found in the CLI bundle: `stale: 60000, update: 5000`, plus a legacy `<dir>.lock`). cclimits mkdir's the same directory, stands down if the CLI holds it, steals it once its mtime is 60s stale, and always rmdir's what it owns. Codex exposes no equivalent lock file (only an in-process mutex), so there it's cclimits-vs-cclimits only.
- **Test isolation bug found & fixed**: `test_oauth_expired` mocked only `http_get`, so the new 401 path reached the developer's **real** `~/.codex/auth.json` and created a lock file there. New autouse `isolated_credentials` fixture in `conftest.py` points `CLAUDE_CRED_PATHS` / `CODEX_AUTH_PATHS` at a temp dir for every test.
- **Verification**: both endpoints probed live with a deliberately invalid refresh token — Claude returns `400 invalid_grant`, Codex `401 token_expired`, i.e. request shape accepted, credential rejected. Then a real forced refresh against the live credential file: access token replaced, refresh token rotated and persisted, expiry advanced, `scopes`/`subscriptionType`/`rateLimitTier` intact, file still 0600, no leftover `.tmp`, and the new token returned HTTP 200 from the usage endpoint.
- **Tests**: 255 total (26 new) — `TestJwtClaims`, `TestWriteJsonSecure`, `TestEnsureClaudeToken` (incl. busy-lock fallback, CLI-lock stand-down, stale-lock steal), `TestEnsureCodexCredentials` in `tests/test_credentials.py`
- **Files**: `lib/cclimits.py`, `tests/test_credentials.py`, `tests/conftest.py`, `README.md`, `memory-bank/deltas.md`, `memory-bank/progress.md`

...
- **tmux wrapper resilience**: `--watch` is a long-running `#()` producer that emits a complete cached line every 2s, avoiding tmux 3.7b's blank interval when short-lived jobs restart; on a cold cache it emits `CCLIMITS_TMUX_PLACEHOLDER` (default `cclimits...`) so tmux never falls back to its own `<'...' not ready>` marker, and on a *failed* self-invocation it emits the distinct `CCLIMITS_TMUX_ERROR` (default `cclimits!`) — a live loop survives its own file being truncated/overwritten, so sharing one marker made a corrupt install look like a slow first lookup. Argument-keyed caches also feed a stable last-known-good display cache. Only *transient* failures (`ERR`/`expired`) are barred from overwriting good output — `no key` is a stable config state and must not block refreshes, or one unconfigured provider would freeze the whole line. Expired Grok triggers one background `grok models`, delegating rotation/write-back to the official CLI without inference — that trigger matches both the `Grok` label and its `--icons` glyph, since matching only the word would silently disable auto-refresh for icon users. `CCLIMITS_TMUX_SEP` restyles `--compact`'s `_` provider separator (e.g. a coloured `┃` matching the pane dividers); the substitution runs on *serve*, never on write, so the cache keeps the raw form that the transient-failure grep depends on and a separator change lands on the next redraw instead of after the TTL
...
- ✅ CI (GitHub Actions matrix) + automated npm publish on `v*` tags via Trusted Publishing (OIDC); 303 tests
